<div class="modal fade" id="modal-form" tabindex="-1" role="dialog" aria-labelledby="modal-form">
    <div class="modal-dialog modal-lg" role="document">
        ...
        <form action="{{ route('perencanaan_proyek.store') }}" method="post" class="form-horizontal" enctype="multipart/form-data">
            @csrf
            @method('post')
                                    ...
                                    <select name="id_persetujuan_pengembangan" id="id_persetujuan_pengembangan" class="form-control" required>
                                        ...
                                        @foreach ($trx_persetujuan_pengembangan as $item)
                                            <option value="{{ $item->id_persetujuan_pengembangan }}">
                                                {{ $item->nomor_dokumen }} - {{ $item->judul }} <!-- Menampilkan nomor dokumen dan judul proyek -->
                                            </option>
                                        @endforeach
                                    </select>
                                    <span class="help-block with-errors"></span>
